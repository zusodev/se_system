...
class TargetUserService extends BaseService
{
    ...
    private $company;
    private $failedTargetUserRepository;

    public function __construct(
        TargetUserRepository $repository,
        TargetDepartmentRepository $targetDepartmentRepository,
        UploadFailedTargetUserRepository $failedTargetUserRepository
    )
    {
        $this->repository = $repository;
        $this->targetDepartmentRepository = $targetDepartmentRepository;
        $this->failedTargetUserRepository = $failedTargetUserRepository;
    }
        ...
        foreach ($records as $offset => $record) {
            try {
                ...
                if ($validator->fails()) {
                    ...
                    $this->createFailRecord($fileName, $createAt, $fields);
                    DB::commit();
                    continue;
                }
    ...
    private function createFailRecord(string $fileName, Carbon $uploadedAt, array $fields)
    {
        $fields['file_name'] = $fileName;
        $fields['uploaded_at'] = $uploadedAt;
        $fields['company_name'] = $this->company->name;
        $this->failedTargetUserRepository->create($fields);
    }
